...
Usage:
    # First time only — compile the 3 NEFFs (one-shot, takes ~1 minute total)
    python -m smol_vla.run_inference --action compile \\
        --hf-checkpoint /path/to/lerobot/smolvla_base \\
        --neff-dir      /path/to/output_neffs

    # Run inference (load NEFFs + benchmark)
...
# Allow running as either `python -m smol_vla.run_inference` (package mode)
# or `python run_inference.py` (script mode from inside the package directory).
try:
...
logger = logging.getLogger("smol_vla.run_inference")
...
def main():
    ...
    parser.add_argument("--hf-checkpoint", required=True,
                        help="Path to lerobot/smolvla_base HF snapshot dir")

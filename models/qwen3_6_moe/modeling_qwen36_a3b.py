...
class NeuronMTPHead(nn.Module):
    ...
    def draft_step(
        ...
        combined = torch.cat(
            [self.hidden_norm(prev_hidden), self.embed_norm(next_input_embeds)],
            dim=-1,
        )
        ...
        return self.mtp_lm_head(self.final_norm(out[0]))
...
def convert_qwen36_a3b_hf_to_neuron_state_dict(neuron_state_dict, config):
    ...
    MTP (single layer):
        mtp.layers.0.{embed,hidden,final}_layernorm -> mtp_head.{embed,hidden,final}_norm
        mtp.layers.0.input_proj -> mtp_head.eh_proj
        mtp.layers.0.lm_head -> mtp_head.mtp_lm_head
        mtp.layers.0.<rest> -> mtp_head.decoder_layer.<rest>
    ...
    for l in range(config.num_hidden_layers):
        ...
        # MoE FFN (every layer): router rename + expert fusion. Router and
        # experts live under `mlp.moe.*` because NeuronMoEBlock wraps NxDI's
        # MoE module as `self.moe`.
        gate_key = f"layers.{l}.mlp.gate.weight"
        if gate_key in neuron_state_dict:
            neuron_state_dict[f"layers.{l}.mlp.moe.router.linear_router.weight"] = (
                neuron_state_dict.pop(gate_key).detach().clone()
            )

        # HF ships routed experts as two fused 3D tensors per layer:
        #   experts.gate_up_proj  shape (E, 2*I, H)
        #   experts.down_proj     shape (E, H, I)
        # NxDI ExpertMLPsV2 wants:
        #   .gate_up_proj.weight  shape (E, H, 2*I)
        #   .down_proj.weight     shape (E, I, H)
        # Just rename + transpose dims (1, 2).
        for hf_suffix, nxdi_suffix in (
            ...
            src = f"layers.{l}.mlp.{hf_suffix}"
            if src in neuron_state_dict:
                t = neuron_state_dict.pop(src).transpose(1, 2).contiguous()
                neuron_state_dict[f"layers.{l}.mlp.{nxdi_suffix}"] = t

        # Shared expert (kept under its singular HF name; owned by NeuronMoEBlock).
        # HF: mlp.shared_expert.{gate,up,down}_proj -> NxDI: mlp.shared_expert.<same>
        # HF: mlp.shared_expert_gate.weight -> NxDI: mlp.shared_expert_gate.weight
        # (Both pass through unchanged -- listed here for documentation.)

        gc.collect()

    # MTP head: remap mtp.layers.0.* (or mtp.0.*) to mtp_head.*. When MTP is
    # disabled in the inference config, drop any mtp.* keys present in the HF
    # checkpoint so they don't trip load_state_dict.
    if config.mtp_num_hidden_layers == 0:
        for k in [k for k in list(neuron_state_dict.keys()) if k.startswith("mtp.")]:
            neuron_state_dict.pop(k)
    else:
        rename = {
            "embed_layernorm.weight": "embed_norm.weight",
            "hidden_layernorm.weight": "hidden_norm.weight",
            "input_proj.weight": "eh_proj.weight",
            "final_layernorm.weight": "final_norm.weight",
            "lm_head.weight": "mtp_lm_head.weight",
        }
        for k in [k for k in list(neuron_state_dict.keys()) if k.startswith("mtp.")]:
            rest = k[len("mtp."):]
            if rest.startswith("layers.0."):
                rest = rest[len("layers.0."):]
            elif rest.startswith("0."):
                rest = rest[len("0."):]
            new_suffix = rename.get(rest)
            if new_suffix is not None:
                neuron_state_dict[f"mtp_head.{new_suffix}"] = (
                    neuron_state_dict.pop(k).detach().clone()
                )
            else:
                neuron_state_dict[f"mtp_head.decoder_layer.{rest}"] = (
                    neuron_state_dict.pop(k).detach().clone()
                )

...
class NeuronGatedDeltaNet(nn.Module):
    ...
    def verify_block_candidates(self, query, key, value, g, beta, initial_state):
        ...
        outputs = []
        states = []
        # GREEDY-EQUIVALENCE (bit-identical state carry vs sequential TKG decode):
        # the TKG decode path (forward()/is_decode) rounds the recurrent state to
        # the buffer dtype (bf16) after EVERY token -- it reads recurrent_state_buffer
        # (bf16) -> float32 -> _recurrent_step -> writes new_state.to(bf16) back. So
        # in plain greedy decode the state fed into token i+1 is the bf16-ROUNDED
        # state after token i. If the verify block keeps the intermediate state in
        # float32 across the 2 block tokens, its S2 (state after the draft token)
        # differs from TKG's S2 in the low bits; that error compounds across spec
        # rounds and eventually flips a committed token (the Stage C.1 oracle caught
        # exactly this: verify's pos-0 logit drifting from reference after ~3 rounds).
        # Match TKG: round the carried state to the buffer dtype between steps. The
        # buffer dtype is float32 inside _recurrent_step's math, so cast through bf16
        # (the recurrent_state_buffer dtype) and back to float32 for the next step.
        ...
        for i in range(num_positions):
            ...
            # Round to the carry (bf16) dtype to match TKG's per-token write-back,
            # then keep the rounded value in float32 for the next step's math. The
            # stored candidate is this rounded state (cast to bf16 in the scratch
            # alias), so the committed seed equals what sequential TKG would carry.
...
class NeuronQwen36A3BModel(NeuronBaseModel):
    ...
    def init_inference_optimization(self, config: Qwen36A3BInferenceConfig):
        ...
        # MTP draft KV PLACEHOLDER (gotcha #6 again, for the renamed draft cache).
        # The MTP draft graph (NeuronMTPDraftModel) owns a private one-layer cache
        # named `draft_kv_mgr.past_key_values.{0,1}` -- a DISTINCT key from the
        # full stack's `kv_mgr.past_key_values.*` so it cannot clobber the live
        # layer-0 GQA buffer the verify graph reads. But NxD's shared
        # StateInitializer zero-allocates state keys from the FIRST traced graph's
        # (CTE) metaneff only; a key that exists solely in the draft graph is never
        # allocated -> "Missing state tensor draft_kv_mgr.past_key_values.0" at
        # load. FIX (mirrors _init_verify_candidate_buffers): build a matching
        # one-layer KVCacheManager HERE on the MAIN model so its
        # `draft_kv_mgr.past_key_values.{0,1}` keys/shapes appear in the CTE/TKG
        # metaneff and get allocated; emit them as zero passthrough in the main
        # forward and alias them so the draft graph binds to the same HBM. The
        # main graphs never read/write it (inert passthrough). Gated by MTP only;
    ...
    def forward(
        ...
        # MTP draft-KV placeholder passthrough (gotcha #6 for draft_kv_mgr). Same
        # rationale as the verify candidates above: emit zero passthrough so the
        # `draft_kv_mgr.past_key_values.{0,1}` keys land in the CTE/TKG metaneff and
        # the shared StateInitializer allocates them for the draft graph. Inert for
        # decode; flag-off (no MTP) -> skipped, decode output layout unchanged.
        if (
            not getattr(self.config, "use_hybrid_cache_manager", False)
...
class Qwen36A3BDecoderModelInstance(DecoderModelInstance):
    ...
    def get(self, bucket_rank, **kwargs):
        ...
        if (
            ...
            # MTP draft-KV placeholder buffers, emitted by the main forward as
            # trailing zero passthrough when MTP is enabled. Aliased AFTER the
            # verify candidates so their state keys (draft_kv_mgr.past_key_values.*)
            # are registered on CTE/TKG and the shared StateInitializer allocates
            # them for the draft graph. Empty / no-op when MTP is disabled.
...
class NeuronQwen36A3BForCausalLM(NeuronBaseForCausalLM):
    ...
    # ------------------------------------------------------------------
    # Stage C host-loop state access helpers
    #
    # NxD's runtime state lives in ``NxDModel.state`` = ``List[Dict[str,
    # Tensor]]`` -- one dict per LOCAL rank, keyed by the parameter's dotted
    # checkpoint name. ``ModelBuilder.build_state_initializer`` builds ONE state
    # dict (from the first graph's metaneff) and binds every graph's INPUT_STATE
    # by checkpoint key, so buffers with the same name across graphs are the SAME
    # physical HBM. The buffers are passed BY REFERENCE into every SPMD bucket
    # model, so an in-place ``.copy_()`` into ``state[rank][key]`` mutates the
    # exact HBM the next forward reads -- writes persist.
    #
    # The helpers below give the Stage C loop:
    #   * _loop_state_dicts(): the per-rank state dicts (resolved once);
    #   * read_loop_state(key): the buffer as one CPU tensor (rank 0; the
    #     candidate/trunk buffers are the values the host inspects with argmax);
    #   * write_loop_state(key, value): in-place .copy_() into every rank's
    #     buffer (used to zero buffers between prompts);
    #   * commit_specdec(cand_idx, num_dn): the rank-LOCAL device-to-device copy
    #     that selects the accepted DeltaNet candidate into the live seed buffer.
    # ------------------------------------------------------------------
    ...
    def commit_specdec(self, cand_idx, num_dn):
        ...
        Per DeltaNet layer ``i`` and per local rank ``r``, a rank-LOCAL
        device-to-device copy:

            recurrent_state_buffer <- recurrent_cand_buffers.i[:, cand_idx]
            conv_state_buffer      <- conv_cand_buffers.i[:, cand_idx]

        Both source (verify-only candidate scratch) and destination (live seed
        buffer the next forward reads) live on the SAME rank with the SAME shard
        layout, so this is a pure rank-local copy -- never a cross-rank gather.
        ``cand_idx`` = accept_count - 1 (0 on reject -> S1, 1 on accept -> S2).

        GQA needs NO commit here: the verify graph already wrote the block KV
        into the shared live cache (it is the sole loop writer). Returns the
        number of (layer, rank) copies performed.
        """
        state = self._loop_state_dicts()
        ...
        for rs in state:
            for i in range(num_dn):
                ...
                rec_seed.copy_(rec_cand[:, cand_idx].to(rec_seed.dtype))
                conv_seed.copy_(conv_cand[:, cand_idx].to(conv_seed.dtype))
                n += 1
        return n
    ...
    def _dn_seed_keys(self, state=None):
        ...
        Returns ``(recurrent_state_keys, conv_state_keys)``, each a list indexed
        by DeltaNet ordinal (0..num_dn-1). The seed buffers are nn.Parameters on
        each layer's ``linear_attn`` so their state key is the dotted module path
        ``layers.{N}.linear_attn.{recurrent,conv}_state_buffer``; we DISCOVER the
        keys from the live state dict (sorted by absolute layer index) rather
        than hardcoding the prefix, so this is robust to any wrapping NxD adds.
        The recurrent/conv candidate buffers and the seed buffers share the same
        DeltaNet layer order (both iterate self.layers' linear_attn), so ordinal
        i lines up across candidate and seed lists. Cached after first call.
        """

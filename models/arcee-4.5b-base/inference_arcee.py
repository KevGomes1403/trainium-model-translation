...
from modeling_arcee_neuron import NeuronArceeForCausalLM
...
def _generation_from_pretrained_compat(cls, pretrained_model_name_or_path, *args, **kwargs):
    """Fallback to model config when generation_config.json is absent."""
    try:
        return _ORIG_GENERATION_FROM_PRETRAINED(pretrained_model_name_or_path, *args, **kwargs)
    except OSError:
        model_path = Path(str(pretrained_model_name_or_path)).expanduser()
        if not model_path.is_dir() or (model_path / "generation_config.json").exists():
            raise

        try:
            model_config = AutoConfig.from_pretrained(pretrained_model_name_or_path, trust_remote_code=True)
        except Exception:
            config_path = model_path / "config.json"
            with open(config_path, "r", encoding="utf-8") as f:
                model_config = PretrainedConfig.from_dict(json.load(f))

        generation_config = cls.from_model_config(model_config)
...
def main():
    ...
    inference_demo.MODEL_TYPES["arcee"] = {"causal-lm": NeuronArceeForCausalLM}
    inference_demo.load_pretrained_config = _load_pretrained_config_compat
    GenerationConfig.from_pretrained = classmethod(_generation_from_pretrained_compat)
